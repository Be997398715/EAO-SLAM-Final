                                                                                                                                                                                              ...
                                                                                                                                                                                              mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    */
    ...
    //yolact segment init
    yolact segmentor(0.5,0.4);
    cout << "Segment YOLACT Create!" <<endl;
}
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp, const bool bSemanticOnline)
{
    ...
    if (bSemanticOnline)
    {
        ...
        #ifdef using_yolact
            mImSeg = imRGB.clone();
            Mat seg_mask = segmentor.detect(mImSeg);
            mCurrentFrame.seg_mask = seg_mask.clone();
        #endif
...
bool Tracking::TrackWithMotionModel()
{
    ...
    for (auto &box : mCurrentFrame.boxes)
    {
        ...
        #ifdef using_yolact
            // copy mask
            obj->mask = mCurrentFrame.seg_mask;
        #endif
    ...
    if ((mCurrentFrame.mnId > mnObjectIniFrameID) && (mbObjectIni == true))
    {
        ...
        cout << "step 10.2 data association."<< endl;
...
void Tracking::CreateNewKeyFrame(bool CreateByObjs)
{
    ...
    cout << "CreateNewKeyFrame start" <<endl;
    // save objects to keyframe for BA
    ...
    for(auto obj:mCurrentFrame.mvObject3dFrame)
    {
        g2o::cuboid cube_ground_value; // offline cuboid txt in local ground frame.  [x y z yaw l w h]
        Vector9d cube_pose;

        cube_pose << obj->mCuboid3D.cuboidCenter[0], obj->mCuboid3D.cuboidCenter[1], obj->mCuboid3D.cuboidCenter[2],
            obj->mCuboid3D.rotP, obj->mCuboid3D.rotR, obj->mCuboid3D.rotY,
            obj->mCuboid3D.lenth, obj->mCuboid3D.width, obj->mCuboid3D.height;
        cube_ground_value.fromMinimalVector(cube_pose);

        obj->bbox_2d = obj->mObjectFrame[0]->mBoxRect;
        obj->bbox_vec = Vector4d((double)obj->bbox_2d.x + (double)obj->bbox_2d.width / 2, (double)obj->bbox_2d.y + (double)obj->bbox_2d.height / 2,
                                       (double)obj->bbox_2d.width, (double)obj->bbox_2d.height);

        obj->SetReferenceKeyFrame(pKF);
        obj->object_id_in_localKF = pKF->local_cuboids.size();

        g2o::cuboid global_obj_pose_to_init = cube_ground_value.transform_from(frame_pose_to_init);
        obj->SetWorldPos(global_obj_pose_to_init);
        obj->addObservation(pKF, obj->object_id_in_localKF);
        pKF->local_cuboids.push_back(obj);
    ...
    if(mSensor!=System::MONOCULAR)
        #ifdef using_yolact
            mpPointCloudMapping->insertKeyFrame( pKF, this->mImSeg, this->mImDep );
        #else
...
void Tracking::AssociateObjAndPoints(vector<Object_2D *> objs_2d)
{
        ...
        if (mCurrentFrame.mvpMapPoints[i])
        {
            ...
            if (!pMP->isBad())
            {
                for (size_t k = 0; k < objs_2d.size(); ++k)
                {
                        ...
                        //if (objs_2d[k]->mBoxRect.contains(mCurrentFrame.mvKeysUn[i].pt) && objs_2d[k]->_class_id==(objs_2d[k]->mask.at<uchar>(mCurrentFrame.mvKeysUn[i].pt.y, mCurrentFrame.mvKeysUn[i].pt.x)-1))// in rect and mask
                        {
    ...
    cout << "used mappoints for object=" << N_used << endl;
...
void Tracking::InitObjMap(vector<Object_2D *> objs_2d)
{
    ...
    for (auto &obj : objs_2d)
    {
        ...
        ObjectMapSingle->mnBALocalForKF = nGoodObjId;   // 3d object in ba optimize
        ...
        //save this 3d object to current frame
        mCurrentFrame.mvObject3dFrame.push_back(ObjectMapSingle);
        // todo: save to key frame in tracking.cc : createnewkeyframe.
