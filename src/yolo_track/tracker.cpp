...
float Tracker_::CalculateIou(const cv::Rect& det, const Track& track) {
...
void Tracker_::HungarianMatching(const std::vector<std::vector<float>>& iou_matrix,
                                size_t nrows, size_t ncols,
                                std::vector<std::vector<float>>& association) {
    Matrix_<float> matrix(nrows, ncols);
...
void Tracker_::AssociateDetectionsToTrackers(const std::vector<cv::Rect>& detection,
...
void Tracker_::Run(const std::vector<cv::Rect>& detections) {
...
std::map<int, Track> Tracker_::GetTracks() {
    return tracks_;
}