...
cout <<"STEP 4. create a new object" << endl;
        ...
        cout << "mvObject3dFrame.push_back" << endl;
...
void Object_Map::addObservation(KeyFrame *pKF, size_t idx)
{
	unique_lock<mutex> lock(mMutexFeatures);
	if (mLatestKF == nullptr)
	{
		mLatestKF = pKF;
		cout << "mLatestKF == nullptr"<< endl;
	}
	mLatestKF = pKF;
...
void Object_Map::EraseObservation(KeyFrame *pKF)
{
	bool bBad = false;
	{
		unique_lock<mutex> lock(mMutexFeatures);
...
void Object_Map::SetReferenceKeyFrame(KeyFrame *refkf)
{
	moRefKF = refkf;
}


unordered_map<KeyFrame *, size_t> Object_Map::GetObservations()
{
	unique_lock<mutex> lock(mMutexFeatures);
	return mObservations;
}

int Object_Map::Observations()
{
	unique_lock<mutex> lock(mMutexFeatures);
	return nObs;
}
...
void Object_Map::SetWorldPos(const g2o::cuboid &Pos)
{
	unique_lock<mutex> lock2(mGlobalMutexObject);
	unique_lock<mutex> lock(mMutexPos);
	pose_Twc = Pos;
	pose_Tcw = pose_Twc;
	pose_Tcw.pose = pose_Twc.pose.inverse();

    this->mCuboid3D.pose = Pos.pose;
    this->mCuboid3D.lenth = Pos.scale[0];
    this->mCuboid3D.width = Pos.scale[1];
    this->mCuboid3D.height = Pos.scale[2];
    this->mCuboid3D.cuboidCenter[0] = Pos.xyzpry[0];
    this->mCuboid3D.cuboidCenter[1] = Pos.xyzpry[1];
    this->mCuboid3D.cuboidCenter[2] = Pos.xyzpry[2];
    this->mCuboid3D.rotP = Pos.xyzpry[3];
    this->mCuboid3D.rotR = Pos.xyzpry[4];
    this->mCuboid3D.rotY = Pos.xyzpry[5];
    this->mCenter3D.at<float>(0) = Pos.xyzpry[0];
    this->mCenter3D.at<float>(1) = Pos.xyzpry[1];
    this->mCenter3D.at<float>(2) = Pos.xyzpry[2];
}

g2o::cuboid Object_Map::GetWorldPos()
{
	unique_lock<mutex> lock(mMutexPos);
	return pose_Twc;
}

g2o::SE3Quat Object_Map::GetWorldPosInv()
{
	unique_lock<mutex> lock(mMutexPos);
	return pose_Tcw.pose;
}

g2o::cuboid Object_Map::GetWorldPosBA()
{
	unique_lock<mutex> lock(mMutexPos);
