...
{
    cout << "LocalBACameraPointObjects start"<< endl;
    ...
    vector<Object_Map *> lLocalMapObjects;
    for (vector<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end(); lit != lend; lit++)
    {
        vector<Object_Map *> vpMOs = (*lit)->local_cuboids;
        for (vector<Object_Map *>::iterator vit = vpMOs.begin(), vend = vpMOs.end(); vit != vend; vit++)
        {
            Object_Map *pMO = *vit;
            if (pMO)
                if (!pMO->bBadErase)
    ...
    for (vector<Object_Map *>::iterator lit = lLocalMapObjects.begin(), lend = lLocalMapObjects.end(); lit != lend; lit++)
    {
    ...
    if (1)
    {
        ...
        {
            Object_Map *pMO = lLocalMapObjects[i];
            pMO->point_object_BA_counter++;
            pMO->used_points_in_BA.clear();
            pMO->used_points_in_BA_filtered.clear();

            const std::vector<MapPoint *> &UniquePoints = pMO->mvpMapObjectMappoints;
        ...
        for (size_t i = 0; i < lLocalMapObjects.size(); i++)
        {
            Object_Map *pMObj = lLocalMapObjects[i];
    ...
    if (1)
    {
        ...
        inv_sigma.setOnes();
        inv_sigma = inv_sigma * 1.0f; // point sigma<1, object error is usually large, no need to set large sigma...
        ...
        if (whether_want_camera_obj)
            for (vector<Object_Map *>::iterator lit = lLocalMapObjects.begin(), lend = lLocalMapObjects.end(); lit != lend; lit++)
            {
                ...
                for (unordered_map<KeyFrame *, size_t>::const_iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
                {
                    ...
                    if (!pKFi->isBad())
                    {
                        const Object_Map *local_object = pKFi->local_cuboids[mit->second];
    ...
    std::cout << "Recover optimized data Object" << std::endl;
}
